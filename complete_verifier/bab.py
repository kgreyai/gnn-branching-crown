...
def split_domain(net: LiRPANet, domains, d, batch, stats=None,
    ...
    # wja: data collection hook (enabled via env var WJA_COLLECT=1)
    _wja_flag = os.environ.get('WJA_COLLECT', 'NOT_SET').strip()
    if _wja_flag == '1':
        try:
            from wja_collect_data import collect_branch_state
            n_batch = next(iter(d['lower_bounds'].values())).shape[0]
            for _b in range(n_batch):
                collect_branch_state(d, branching_decision, split_depth, net, _b)
        except Exception as _wja_e:
            print(f'[wja collect ERROR] {_wja_e}')
    elif _wja_flag == 'NOT_SET':
        pass  # env var not set, silent
    else:
        print(f'[wja] WJA_COLLECT={repr(_wja_flag)} (expected "1")')
...
def general_bab(net: LiRPANet, x, c, rhs,
    ...
    # wja: set problem_id for data collection (one call = one verification problem)
    if os.environ.get('WJA_COLLECT', '').strip() == '1':
        try:
            from wja_collect_data import init_collector
            _pid = str(arguments.Globals.get('example_idx', 'unk'))
            _save_dir = os.environ.get('WJA_SAVE_DIR', './branch_data').strip()
            init_collector(_save_dir, _pid)
        except Exception:
            pass
