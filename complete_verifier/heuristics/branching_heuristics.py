...
def get_branching_heuristic(net, method=None):
    ...
    elif branching_method == 'gnn':
        from heuristics.gnn import GnnBranching
        model_path = arguments.Config['bab']['branching'].get('gnn_model_path', 'gnn_branch_model.pt')
        fallback = arguments.Config['bab']['branching'].get('gnn_fallback', 'babsr')
        branching_obj = GnnBranching(net, model_path=model_path, fallback=fallback)
    else:
